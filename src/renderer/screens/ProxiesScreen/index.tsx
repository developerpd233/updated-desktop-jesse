...
const ProxiesScreen = () => {
  ...
  const createData = (
    ...
    return { id, proxy: proxy.substring(0, 35), status, action };
  };
  ...
  async function fetchMyAPI() {
    ...
    if (result.type === 'PROXY_LIST_SUCCESS') {
      ...
      result.response.data.data?.map((data: any, index: number) => {
        if (data.proxy.length > 15) {
          data.proxy.substring(0, 15);
        }
        obj = createData(
          index++,
          data.proxy,
  ...
  return (
    ...
    >
      <Box display={'flex'} justifyContent={'flex-end'} flexDirection={'row'}>
        ...
        >
          <Box sx={{ marginBottom: 5 }}>
            <CssGradientButton type="submit" onClick={createNewProxy}>
              New Proxies
            </CssGradientButton>
          </Box>
        </Grid>
      ...
      >
        <TableComponent
          columns={proxiesolumns}
          rows={rows}
          loader={getAllProxyReducer?.loading}
        />
      </Box>
