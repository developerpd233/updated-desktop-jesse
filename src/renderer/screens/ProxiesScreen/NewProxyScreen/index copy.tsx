...
import {
  ...
  getProxyRecord,
  updateProxy,
} from '../../../redux/actions/proxy-action';
...
const NewProxyScreen = () => {
  ...
  const onSubmit = async (e: any) => {
    ...
    for (let [key, value] of Object.entries(values)) {
      if (value.length === 0) {
        errorObject[key] = `Please enter a ${key.replace(
          /([a-z])([A-Z])/g,
          '$1 $2'
        )} value`;
        setFlag(!flag);
  ...
  const onUpdate = async (e: any) => {
    ...
    if (Object.entries(errorObject).length === 0) {
      ...
      let result: any = await dispatch(
        updateProxy(record, sessionStorage.getItem('proxyId'))
      );
  ...
  const fetchData = async () => {
    let result: any = await dispatch(
      getProxyRecord(sessionStorage.getItem('proxyId'))
    );
          ...
          >
            {!addNewProxyReducer.loading ? (
              <Grid
                container
                columnSpacing={3}
                sx={{ marginBottom: 5, paddingLeft: 2, paddingRight: 2 }}
              >
                ...
                >
                  <Button
                    onClick={
                      !!sessionStorage.getItem('proxyId') ? onUpdate : onSubmit
                    }
                    variant="contained"
                    sx={{
                      borderRadius: 30,
